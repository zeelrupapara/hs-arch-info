...
| Category | Evidence Type | File / Link | Description |
|----------|---------------|-------------|-------------|
| API: Order Placement | Screenshot | API gateway config | showing rate limits and auth middleware |
| API: Order Placement | Sample request/response JSON log |  | captured via Postman |
| DB Schema: Orders | DDL dump | schema_dump.sql |  |
| DB Schema: Orders | ERD diagram | vfxcore.png | highlighting foreign keys and indexes |
| Event Topic: system.order | Output | grep nats.Publish | from source code |
| Event Topic: system.order | Broker configuration screenshot |  | showing subject subscriptions |
| Validation Rule: Max Order Size | Code snippet | order_validation.go |  |
| Validation Rule: Max Order Size | Test execution log | validation_test.log | showing rejection when limit exceeded |
| LP Config: LPABC | Config file | lp_config.docx | showing FIX parameters |
| LP Config: LPABC | Monitoring dashboard screenshot | monitoring_dashboard.png | displaying connection status |
| Report: CAT | Sample CAT report | cat_report.csv |  |
| Report: CAT | Email delivery screenshot |  | showing transmission to compliance distribution list |
| Report: Account Statement | Generated CSV | account_statement.csv |  |
| Report: Account Statement | Report dashboard screenshot |  |  |
| Report: Online Session | Session log | session_log.csv |  |
| Report: Online Session | Screenshot |  | of report execution from UI dashboard |
